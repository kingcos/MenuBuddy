...
- **5 种经验来源**：抚摸（5 XP）、每日登录（20 XP）、系统事件（3 XP）、应用切换（1 XP）、AI 反应（8 XP）
- **等级公式**：`等级 = floor(sqrt(XP / 10))`——从 Lv.1（10 XP）到 Lv.10（1000 XP）的平滑曲线
- **属性加点**：每升一级获得 +2 属性点，可分配到任意属性
- **更换角色**：Lv.5+ 后可在物种图鉴中自由切换物种
- **升级庆祝**：弹出通知展示新解锁的槽位和装扮奖励

### 装扮系统（v2）

用可插拔的物品系统自定义伙伴的外观：

- **5 个装扮槽位**：帽子、眼睛、配饰、光环、边框——随等级逐步解锁（Lv.0 → Lv.8）
- **40+ 物品**，5 种稀有度（普通 → 传说），每件物品都有 ASCII 精灵修饰效果
- **随机掉落**：每次抚摸 5% 概率 + 升级必得奖励
- **自定义帽子创作器**：设计 12 字符的 ASCII 帽子，实时预览
- **导入 / 导出**：通过 base64 分享码在用户间分享物品
- **装扮界面**：槽位标签页、物品网格、装备/卸载、实时精灵预览

> 完整设计文档见 [docs/v2-progression-and-cosmetics.md](docs/v2-progression-and-cosmetics.md)。
